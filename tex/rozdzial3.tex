...
\begin{itemize}
	\item Okno wyboru autora
	\item Okno wyboru albumu
	\item Okno wyboru utworów
\end{itemize}
...
\paragraph{Albumy} \label{sec:dbalbums}
...
Dlatego, że miniaturki mają być dostępne w wielu miejscach w aplikacji, należałoby umieścić odnośniki do nich w bazie danych, konkretnie w tabeli dotyczącej albumów (patrz sekcja nr.~\ref{sec:dbalbums}) - logicznie, jeżeli miniaturka danego albumu jest dostępna, będzie używana w wielu podobnych kontekstach gdzie używane są same informacje o albumie. 

Pozostaje kwestia faktycznego pozyskiwania obrazów z pliku. Proces ten odbędzie się podczas etapu ładowania tagów, po załadowaniu faktycznych plików, ale przed załadowaniem informacji do bazy. Miniaturki zostaną wyciągnięte z plików, i zostaną one włożone do głównego katalogu aplikacji jako bitmapy. Przechowywanie miniaturek jako osobne pliki pozwala na łatwiejsze ich pozyskiwanie podczas dalszego działania aplikacji.
