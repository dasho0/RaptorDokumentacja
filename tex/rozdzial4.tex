...
Logika audio player znajduje się w pliku AudioPlayer.kt.
Działanie rozpoczyna od inicjalizacji ExoPlayer przedstawionej na listingu nr \ref{lst:player-exoinit}.
\begin{lstlisting}[caption=Inicjalizacja exoplayer, label={lst:player-exoinit}, language=kotlin]
	...
	\end{lstlisting}
	setAudioAttributes ustawia atrybuty odtwarzania audio a ExoPlayer.Builder tworzy nową instancję odtwarzacza.
	Następnie w kodzie znajduje się funkcja \texttt{updateListeners}, przedstawiona na listing nr \ref{lst:player-update}.
	Funkcja jest odpowiedzialna za zarządzanie stanem odtwarzania, używa obserwowanego stanu odtwarzacza \texttt{playbackState} i słuchacza, aby reagować na zmiany w stanie odtwarzania.
	\begin{lstlisting}[caption=Zarządzanie stanem, label={lst:player-update}, language=kotlin]
	...
	\end{lstlisting}
	Funkcja wewnętrzna \texttt{onIsPlayingChanged} jest odpowiedzialna za wykrywanie czy odtwarzacz gra oraz aktulaizowanie stanu. Funkcja \texttt{onPlaybackStateChanged} jest odpowiedzialna za reagowanie na zmiany stanu exoplayera.
	\texttt{playbackState} przechowuje stan.
	Następnie jest funkcja \texttt{playSong} odpowiedzialna za ustawienie pliku audio do odtwarzania oraz rozpoczęcie odtwarzania, przedstawiona na listingu nr \ref{lst:player-playsong}.
	\begin{lstlisting}[caption=Funkcja playsong, label={lst:player-playsong}, language=kotlin]
	...
	\end{lstlisting}
	\texttt{MediaItem.fromUr} tworzy element multimedialny z URI pliku audio. \texttt{setMediaItem} ustawia element multimedialny do odtwarzania. \texttt{prepare} przygotowuje odtwarzacz do odtwarzania. \texttt{play} rozpoczyna odtwarzanie.
	Funkcje \texttt{pause} oraz \texttt{restartCurrentPlayback} przedstawione na listingu nr \ref{lst:player-pausecurr} odpowiedzialne są za pauzowanie oraz ponownym odtwarzaniem.
	\begin{lstlisting}[caption=Funkcje pausei restartCurrentPlayuback, label={lst:player-pausecurr}, language=kotlin]
	...
	\end{lstlisting}
	Funkcja \texttt{changeCurrentPosition} przedstawiona na listingu nr \ref{lst:player-position} odpowiedzialna jest za zmanę aktualnej pozycji odtwarzania.
	\begin{lstlisting}[caption=Funkcja zmainy pozycji, label={lst:player-position}, language=kotlin]
		fun changeCurrentPosition(time: Long) {
			player.seekTo(time)
		}
	\end{lstlisting}
	\texttt{seekTo} przenosi odtwarzacz do konkretnego miejsca.
	Na samym końcu znajduje się funkcja \texttt{releasePlayer} która jest odpowiedzialna za czyszczenie pamięci, przedstawiona jest na listingu nr \ref{lst:player-release}
	\begin{lstlisting}[caption=Funkcja czyszczenia pamięci, label={lst:player-release}, language=kotlin]
		fun releasePlayer() {
			player.release()
		}
	\end{lstlisting}
